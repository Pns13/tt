...
def banner():
	...
	cik2=mark(wel ,style='lightpurple')
	sol().print(cik2)
	ban='''
�   AUTHOR  : Mr. PNS  �  
...

                                         VERSISION :4.0.1       
'''
	oi = nel(tekz(ban,justify='center',style='bold'), style='lightpurple')
	cetak(nel(oi, title='[bold lightpurple] � KENDAK KABA LA �  [/bold lightpurple]'))
# VALIDASI TOKEN
def login():
		try:
			...
			try:
				...
				sol().print(lo, style='lightpurple')
				exit()
		except IOError:
			login_lagi334()
...
[05] GROUP MEMBERS           [00] LOGOUT[bold lightpurple]'''
	oi = nel(io, style='cyan')
	cetak(nel(oi, title='[bold lightpurple] • MENU CRACK • [/bold lightpurple]'))
	...
	if ec in ['1','01']:
		...
		time.sleep(1)
		sw = '# SUCCESS OUT'
		sol().print(mark(sw, style='lightpurple'))
		exit()
	else:
...
def result():
	...
	kis = nel(kayes, style='lightpurple')
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			gerr = '# YOUR CP RESULT'
			sol().print(mark(gerr, style='lightpurple'))
			cih = 0
			lol = {}
			for isi in vin:
			...
			sol().print(mark(akun, style='lightpurple'))
			nocp=0
...
def file():
	...
	sol().print(mark(tek, style='lightpurple'), style='on red')
	print(k+'['+h+'•'+k+'] READING THE FILE, WAIT A MINUTE •••')
	time.sleep(2)
	teks = '# SELECT FILES TO CHECK'
	sol().print(mark(teks, style='lightpurple'))
	my_files = []
	try:
	...
	if len(my_files)==0:
		...
		sol().print(mark(teks2, style='lightpurple'))
		geeh = input(k+'['+p+'f'+k+'] Choose : ')
		try:geh = lol[geeh]
		except KeyError:
			ric = '# OPTION NOT IN THE MENU'
...
def dump_massal():
	...
	mas2=nel(mas,style='lightpurple')
...
def dump_grup():
	...
	try:
		...
		au2 = mark(win, style='lightpurple')
		sol().print(au2)
		time.sleep(0.5)
		exit()
	titt = res.find("title")
	...
	oi = nel(au, style='lightpurple')
	cetak(nel(oi, title='[bold lightpurple] • GROUP TARGET •[/bold lightpurple]'))
	au = '[•] TO STOP PRESS CTRL+C\n[•] IF STUCK ON OF AIRPLANE MODE'
	oi = nel(au, style='cyan')
	cetak(nel(oi, title='[bold lightpurple] • SUGGESTION •[/bold lightpurple]'))
...
def setting():
	...
	tak = nel(teks, style='lightpurple')
	...
	gess = nel(ioz, style='lightpurple')
	...
	sol().print(mark(guw, style='lightpurple'))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					infoakun += (f"\n[bold lightpurple][•] LIST ACTIVE APPLICATIONS :[/bold lightpurple] \n")
					apkaktif=re.findall('</i><div class=".*?"><span class=".*?">(.*?)</span><div></div><div class=".*?">(.*?)</div></div>',str(cek))
					nok=1
					for muncul in apkaktif:
						infoakun+= (f"[bold lightpurple][{nok}] {muncul[0]} {muncul[1]}[/bold lightpurple]\n")
						nok+=1
